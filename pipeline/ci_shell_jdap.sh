...
for _aliapi_command in openssl curl; do
    if ! command -v $_aliapi_command &> /dev/null; then
        echo "JdCloud OpenAPI SDK: $_aliapi_command command not found"
        exit 127
    fi
done
...
# jdapi_rpc <http_method> <host> <api_version> <api_action> [<--key> <value>...]
jdapi_rpc() {
    _aliapi_check_vars || return $?

    if [[ $# -lt 4 ]];then
        echo "jdapi_rpc: not enough parameters" >&2
        return 2
    fi
    ...
    do
        case $1 in
            --*)
                if [[ $# -le 1 ]]; then
                    echo "jdapi_rpc: '$1' has no value" >&2
                    return 2
                fi
                _api_params[${1:2}]="$2"
                shift 2
                ;;
            *)
                echo "jdapi_rpc: '$1' is unknown parameter" >&2
                return 2
                ;;
        esac
    done
    ...
    #local _k_date_base64=$(echo -n "$_x_jdcloud_date_simple" | openssl sha256 -hmac "JDCLOUD2$_JdapAccessKeySecret" -binary | openssl base64 -e -A)
...
_aliapi_check_vars() {
    if [[ ! -v JdapAccessKeyId || ! -v JdapAccessKeySecret ]]; then
        echo "JdCloud OpenAPI SDK: 'JdapAccessKeyId' or 'JdapAccessKeySecret' environment variable not found" >&2
        return 3
    fi
}
...
'
    ...
    echo -n "$_sign_str" | openssl sha256 -hmac "$_JdapAccessKeySecret&" -binary | openssl base64 -e
}
...
'
    ...
    _query_str=${_str//$_newline/&}
    _sign_str="$_http_method&$(_aliapi_urlencode "/")&$(_aliapi_urlencode "$_query_str")"
    echo -n "$_sign_str" | openssl sha1 -hmac "$_JdapAccessKeySecret&" -binary | openssl base64 -e
}
...
jdapi_rpc GET cas.aliyuncs.com 2018-07-13 DescribeUserCertificateList --CurrentPage 1 --ShowSize "get_show_size()"
