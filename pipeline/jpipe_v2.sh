...
#去除首尾的空格
gaia_jdap_ak=`echo ${{cred.${{parameters.jdap_ak_sk}}.username}}`
gaia_jdap_sk=`echo ${{cred.${{parameters.jdap_ak_sk}}.password}}`
gaia_jdap_project_id=`echo ${{parameters.jdap_project_id}}`
gaia_jdap_app_id=`echo ${{parameters.jdap_app_id}}`
gaia_jdap_group_id=`echo ${{parameters.jdap_group_id}}`
gaia_jdap_image_url=`echo ${{parameters.image_url}}`
gaia_jdap_component_name=`echo ${{parameters.component_name}}`

cd /
./jdap-connector-jpipe deploy-appgroup  --user-ak=${gaia_jdap_ak} --user-sk=${gaia_jdap_sk} --jdap-project-id=${gaia_jdap_project_id} --jdap-app-id=${gaia_jdap_app_id} --jdap-group-id=${gaia_jdap_group_id} --image-url=${gaia_jdap_image_url} --component-name=${gaia_jdap_component_name}
...
deploy_view_detail_route=`echo ${concerned_result_json} |  jq -r '.data.viewDetailRoute // empty'`
...
if [ ${deploy_result_code} == 0 ];then
  ...
  JDAP_DEPLOY_URL="${GAIA_HOME_URL}${deploy_view_detail_route}"
  #if [${deploy_view_detail_route} == ""] then
